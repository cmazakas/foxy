...
private:
  struct state
  {
    ...
    explicit state(ReadHandler const&             handler,
...
public:
  ...
  read_op(::foxy::basic_session<Stream>& session,
          Parser&                        parser,
           ...
           bool const                is_continuation) -> void
{
  ...
  BOOST_ASIO_CORO_REENTER(*this)
  {
    ...
    http::async_read(s.session.stream, s.session.buffer, s.parser,
                     std::move(*this));
                                                     ...
                                                     std::size_t))
{
  ...
  detail::timed_op_wrapper<Stream, detail::read_op,
                           BOOST_ASIO_HANDLER_TYPE(
                             ReadHandler,
                             void(boost::system::error_code, std::size_t)),
                           void(boost::system::error_code, std::size_t)>(
    *this, std::move(init.completion_handler))
    .template init<Stream, Parser>(parser);

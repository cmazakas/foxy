// ruthlessly and shamelessly steal the open source Beast code hosted at:
// https://github.com/boostorg/beast/blob/18447553caca6b9054e77ed0719d85f2bb24be00/example/common/detect_ssl.hpp
//
// specific commit link for the sake of stability provided that github remains up and running
// likely to be outdated
//
#ifndef FOXY_DETAIL_DETECT_SSL_HPP_
#define FOXY_DETAIL_DETECT_SSL_HPP_
...
boost::tribool
detect_ssl(SyncReadStream& stream, DynamicBuffer& buffer, boost::beast::error_code& ec)
{
...
public:
  ...
  //
  // ^
  // note that Foxy had to modify this to make it all work
  // we added the move constructor
  //
...
void
detect_ssl_op<AsyncStream, DynamicBuffer, Handler>::operator()(boost::beast::error_code ec,
                                                               std::size_t bytes_transferred)
{
  ...
  BOOST_ASIO_CORO_REENTER(*this)
  {
    ...
    if (!boost::indeterminate(result_)) {
      ...
      boost::asio::post(stream_.get_executor(), beast::bind_handler(std::move(*this), ec, 0));
    } else {
      // Loop until an error occurs or we get a definitive answer
      for (;;) {
        // The algorithm should never need more than 4 bytes

...
public:
  ...
  sp_alloc_ptr(T* p) BOOST_SP_NOEXCEPT : p_(const_cast<typename boost::remove_cv<T>::type*>(p)) {}
#endif
  ...
  P
  ptr() const BOOST_SP_NOEXCEPT
  {
    return p_;
  }

  BOOST_STATIC_CONSTEXPR std::size_t
                         size() BOOST_SP_NOEXCEPT
  {
    return 1;
  }
  ...
  {
    return sp_alloc_ptr(
      1, std::pointer_traits<P>::pointer_to(const_cast<typename boost::remove_cv<T>::type&>(v)));
  }
#endif
...
public:
  ...
  {
    return sp_alloc_ptr(
      n_, std::pointer_traits<P>::pointer_to(const_cast<typename boost::remove_cv<T>::type&>(v)));
  }
#endif
...
public:
  ...
  {
    return sp_alloc_ptr(
      N, std::pointer_traits<P>::pointer_to(const_cast<typename boost::remove_cv<T>::type&>(v)));
  }
#endif
...
template <class T, class P>
inline bool
operator==(const sp_alloc_ptr<T, P>& lhs, detail::sp_nullptr_t) BOOST_SP_NOEXCEPT
{
  return !lhs.ptr();
}
...
template <class T, class P>
inline bool
operator==(detail::sp_nullptr_t, const sp_alloc_ptr<T, P>& rhs) BOOST_SP_NOEXCEPT
{
  return !rhs.ptr();
}
...
template <class A>
inline void
sp_alloc_clear(A& a, typename sp_alloc_type<A>::type p, std::size_t, boost::false_type)
{
...
template <class A>
inline void
sp_alloc_clear(A& a, typename sp_alloc_type<A>::type p, std::size_t n, boost::true_type)
{
...
class alloc_deleter
  : empty_value<typename detail::sp_alloc_to<A, typename detail::sp_alloc_value<T>::type>::type> {
  typedef typename detail::sp_alloc_to<A, typename detail::sp_alloc_value<T>::type>::type allocator;
  typedef empty_value<allocator>                                                          base;

public:
  typedef detail::sp_alloc_ptr<T, typename detail::sp_alloc_type<allocator>::type> pointer;

  explicit alloc_deleter(const allocator& a) BOOST_SP_NOEXCEPT : base(empty_init_t(), a) {}
...
class sp_alloc_make {
public:
  typedef typename sp_alloc_to<A, typename sp_alloc_value<T>::type>::type allocator;
...
template <class T, class A>
inline
  typename enable_if_<is_unbounded_array<T>::value, std::unique_ptr<T, alloc_deleter<T, A>>>::type
...
inline
  typename enable_if_<is_unbounded_array<T>::value, std::unique_ptr<T, alloc_deleter<T, A>>>::type
  allocate_unique(const A& alloc, std::size_t size, const typename remove_extent<T>::type& value)
{
  detail::sp_alloc_make<T, A> c(alloc, size);
  boost::alloc_construct_n(c.state(), boost::first_scalar(c.get()),
